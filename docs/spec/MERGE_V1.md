...
fixed accepted head is `A`. Preconditions, checked in order before any
composition:

1. `A`, `B`, and `O` share one `workspace_id` and one `schema_epoch_id`,
   else `MERGE_WORKSPACE_MISMATCH` or `MERGE_EPOCH_MISMATCH`;
2. `O` is an ancestor of both `A` and `B`: the caller supplies the two
   bounded S20-500 ancestries (head first) and
   `find_common_ancestor(ours_ancestry, theirs_ancestry)` returns the first
   entry of `ours_ancestry` whose transaction identity also appears in
   `theirs_ancestry`; when none appears within the bounds the result is
   `MERGE_NO_COMMON_ANCESTOR`, and when the supplied `O` differs from that
   entry the result is `MERGE_ANCESTOR_MISMATCH`;
3. `dA = compare(O, A)` and `dB = compare(O, B)` succeed, else
   `MERGE_COMPARE_FAILED` with the exact `COMPARE_*` code preserved;
...
| J2 | both `Added` or both `Changed`/`MetadataOnly`/`Retyped` with the same target object | convergent: that object |
| J3 | `Removed` on one side and any other class on the other | conflict `DeleteEdit` |
...
as above. Its object is built from that body with the label of `A`'s
object and no fingerprint claim; the frozen S20-390 commit revalidates it.
...
The plan is the S20-350 candidate that moves `A` to the merged root, in
raw-ID order of the affected identities, with one operation per identity:

| Situation | Class | Precondition |
|---|---|---|
| bound in merged, not in `A` | `CreateEntity`, or `AddEntryPoint` for kind 16 | `ExpectedIdentityAbsent` |
...
the entity in `O` (or of the added entity, `0` for `Closure`), the field tag
for `FieldEdit` (`0` otherwise), `A`'s and `B`'s objects (`zero32` when
absent), and `detail`: the `IMPACT_*` numeric code for `Closure`, `0`
otherwise. Reason tags:
...
A `Closure` conflict carries the failing identity when the judgment names
one and `zero32` otherwise. A decoder never sorts or repairs input; the
stored bytes re-encode byte-identically.
...
read, per composed field, per collateral check, and per plan operation;
exhaustion is `MERGE_RESOURCE_LIMIT` with no partial result.
...
`IMPACT_*`, `TXN_*`, `BRANCH_*`, and `SCB_*` failures are preserved with
their exact codes inside `MERGE_COMPARE_FAILED`, the commit path, and
`MERGE_CONFLICT_FORMAT_INVALID`, never remapped.
...
- a frozen merge corpus under `conformance/merge/v1/` with at least:
  ...
  conflict reason; a collateral conflict a naive entity comparison would
  accept; a fast-forward-equivalent merge; an empty merge; and the
  symmetric pair of one merged and one conflicting case;
...
- an S20-700 persistent libFuzzer target over the merge judgment and the
  conflict decoder (the eleventh Section 18.5 surface);
