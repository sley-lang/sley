...
enumeration, advancement, and restricted ref recovery. The lock order is:

1. future repository-wide recovery ownership, when S20-530 defines it;
2. `locks/refs.lock`;
3. the S20-390 transaction/fixed-head lock when verified revision loading needs
   it.
