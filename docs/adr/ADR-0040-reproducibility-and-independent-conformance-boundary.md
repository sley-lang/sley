...
Status: proposed; the S20-730 contract is a draft at revision 11 with
...
Note (revision 7, 2026-09-15; reworded at revision 10): the tracked report carried a second-host attestation minted through the contract's section 5.1 runbook between `6a2eef7` and `22970457`; each re-mint since a7852028 is single-host again until the lab lane re-attests, so the current state is the tracked report, and decision 1's "names the gated second host" describes the single-host case.
...
host (`secondary-host`) is reachable but gated: the operator posture allows it
...
Note (revision 11, 2026-09-18): the `superseded_attestations` listing persists across plain rebuilds and is retired per host only by that host's re-attestation of the current commit; the hermetic report gate checks the listing's shape.
