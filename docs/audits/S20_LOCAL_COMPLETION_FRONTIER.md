...
| Succession benchmark | Full S20-600/S20-610 and S20-620 | The verified legacy adapter and offline raw claim chain are mechanics only. Approved fixtures, containment, live adapters, artifact/oracle/accounting verification, the Sley 2 trial runner, and real trials are absent; the protocol and CLI now exist under draft contracts. |
...
- The full S20-250 entity bodies are implemented (`e78a1ab`).
  ...
  (`docs/audits/S20_420_JSON_BRIDGE_CLOSEOUT.md`) and S20-430 is implemented
  (`docs/audits/S20_430_THIN_CLI_CLOSEOUT.md`), so the S20-620 Sley 2 trial
  runner is the next dependency-complete package.
...
- The protocol, JSON bridge, CLI, comparison, and merge production modules
  exist under draft contracts; no `sley-bench` Sley 2 trial runner exists.
...
boundary drifts. The next action is S20-620: the Sley 2 trial runner in
`sley-bench` with a capsule-and-affordance-only agent arm and a complete
trace, mechanics only until a real trial is authorized, while the pending
S20-250, S20-510, S20-520, S20-300, S20-310, S20-320, S20-400, S20-330,
S20-420, and S20-430 reviews land as revisions; nothing in it is treated
as GA.
