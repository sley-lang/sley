...
| Mutation and transactions | Full S20-360/S20-390 | Fresh validation now judges every operation of families E1 through E6 through the VM owner (`docs/adr/ADR-0044-candidate-operation-analysis-through-the-vm-owner.md`), and fixed-head atomic commit passes for executable programs with no selected tests. The five E7 opcodes stay unowned, and selected-test evidence, policy/epoch transitions, and runtime authority remain absent. |
...
- The full S20-250 entity bodies are implemented (`e78a1ab`).
  ...
  the SMP1 appendix C revision 8 execute profile selector is implemented
  (`docs/audits/S20_410_SMP1_FRAME_CLOSEOUT.md` revision 8 addendum), and
  S20-730 mechanics are implemented under
  `docs/spec/REPRODUCIBILITY_AND_INDEPENDENT_CONFORMANCE_V1.md` revision 1
  (campaign
  `machineresearch/sley-2.0/s20-730-reproducibility-independent-conformance-campaign-2026-09-03.md`),
  the standards SBOM and release provenance are implemented under
  `docs/spec/STANDARDS_SBOM_AND_PROVENANCE_V1.md` revision 2 (campaign
  `machineresearch/sley-2.0/s20-710-full-standards-sbom-provenance-campaign-2026-09-03.md`),
  ...
  operation analysis (ADR-0044) and the S20-390 extended semantic profile
  (ADR-0045) are implemented, so hardening the new VM surface with extended
  persistent fuzz lanes is the next dependency-complete work; every other
  remaining boundary waits on a Council review, a schema epoch decision, or
  operator authority for benchmark spend and publication.
...
boundary drifts. The next action is the extended VM fuzz lanes: the
`vm_canonical_inputs` persistent target still exercises the extended profile
only through the Boolean fixtures of slice E1, so families E2 through E6 carry
no adversarial coverage, while the pending S20-250, S20-510,
