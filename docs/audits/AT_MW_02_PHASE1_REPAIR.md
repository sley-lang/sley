...
at `aff5164` unchanged. Accepts Ariadne findings A1-A4 (Nabu concurs on
A2 design). No protocol, session, bridge, CLI, or runner changes.
...
revision view, decoded request, and selected ceilings into the plan. The
lifetime binds the backing revision slices. `encode_entity_read_response`
consumes the plan plus `SessionId` only. All binding, epoch, parameter
relationship, and preflight checks run in preparation; the redundant
`verify_plan` (including its early Signature return) is removed. Read-only
scalar accessors: `work_units`, `body_len`, `object_count`. The plan
reserves no work and certifies no session authority; reservation and live
session/root admission stay with later protocol work. The thin
verified-repo adapter borrows the local owned `VerifiedRevision`.
...
tokens, and budget are unchanged.

## Evidence

Fail-before receipt (6 discriminating failures, exit 101):
`phase1-repair/fail-before.json`. Failing: independent canonical
assembly, inner length defects, zero ceilings, updated zero assertions in
the ceiling matrix, oversized object, single exact allocation.
Pre-fix exposures (passed, defect demonstrated, then removed with the old
API): encoder accepted a substituted expected root and mutated work
without length error. The `verify_plan` early return is structural
evidence at former `entity_read.rs:743`; no executable claim is made for
it. Syntax or compiler errors were never counted as reproduction.
...
-D warnings`, clean). New coverage: nonmonotonic parameter declaration
order, parameter ObjectId/epoch preparation rejection, immutable
request/view capture, byte-ceiling floor with one-under refusal,
over-ceiling refusal before copy, exact-capacity single allocation.
