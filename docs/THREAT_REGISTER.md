...
|---|---|---|---|---|
| T07 | `ID_DUPLICATE_ENTITY` | `CANDIDATE_IDENTITY_COLLISION` | `crates/sley-policy/src/candidate_validation.rs` phase 4, live-binding branch | `t07_live_bound_target_identity_collides_on_the_live_binding_branch` seeds a base state that already binds the derived identity (live-binding branch, no tombstone); `tombstones_graph_errors_and_missing_references_are_distinct` seeds the tombstone branch |
...
| T20 | `SCHEMA_SELF_MODIFICATION` | `POLICY_ISOLATION_SCHEMA_EPOCH_CHANGED` (the expected symbol is dead: never returned) | `crates/sley-policy/src/lib.rs` isolation judgment | `policy_self_oracle_and_protected_entity_isolation_is_pure` in `crates/sley-policy/src/lib.rs`, which seeds the epoch change |
...
| T41 | `PACK_INVALID` | `PACK_CANONICAL_ORDER`, `PACK_DIGEST_MISMATCH`, `PACK_DIGEST_TREE_MISMATCH`, `PACK_DUPLICATE_ENTRY`, `PACK_OBJECT_CORRUPT`, `PACK_OBJECT_MISSING`, `PACK_OBJECT_UNEXPECTED`, `PACK_PROFILE_UNSUPPORTED`, `PACK_RESOURCE_LIMIT`, `PACK_ROOT_INVALID`, `PACK_SCHEMA_UNSUPPORTED`, `PACK_VERSION_UNSUPPORTED` (the `PackErrorCode` family) | `crates/sley-repo/src/lib.rs` pack importer | the `repository_pack_importer` fuzz target and `conformance/repository-pack/v1/rejected.json` |
...
| T16 | `MUTATION_RESOURCE_LIMIT` | `SCB_RESOURCE_LIMIT` when a candidate's operations or preconditions exceed the full validation profile's ceilings; `CFG_RESOURCE_LIMIT` for the program graph, which is the S20-220 owner's own code | `crates/sley-mutate/src/candidate.rs`, `crates/sley-policy/src/candidate_program.rs` | the mutation-candidate conformance corpus and the `mutation_candidate` fuzz target |
...
| T33 | `VM_NONDETERMINISM_DETECTED` | structural: determinism is a property of the encoding rather than a detected failure. Canonical NaN, negative-zero folding, map order by encoded key bytes, and the absence of host state leave nothing nondeterministic to admit; a divergence surfaces as a cache-key or vector mismatch. Map order is a precondition on supplied values rather than a property the VM can derive, so the VM verifies it against the codec at its own boundary: `VM_EXEC_INPUT_NOT_CANONICAL` (27006) for an execution input and `VM_LOWER_IMMEDIATE_MISMATCH` for a constant `constant_ref` or `global_get` names | `crates/sley-vm/src/extended.rs`, `crates/sley-vm/src/execute.rs` | the independent oracle's `check-vm-extended`, which re-derives every accepted vector's 224-byte cache-key preimage, and `e4_maps_differing_only_in_entry_order_never_reach_equality_or_hashing` |
...
| T40 | `GC_REACHABILITY_VIOLATION` | `GC_REACHABILITY_VIOLATION`, the deleter's guard in `delete_planned_candidates` | `crates/sley-repo/src/gc.rs`, checked before every read and unlink of a planned candidate | `t40_reachable_candidate_in_a_corrupted_plan_trips_the_guard_before_any_delete` seeds a reachable object into the plan and proves the run deletes nothing; `t40_seed_reachable_candidate_assertion_is_effective` covers the planner |
| T48 | `CAPABILITY_DENIED` | `POLICY_GRANT_DENIED` and `CAPABILITY_SUMMARY_MISMATCH` (phase 9 of candidate validation: authority comes only from the protected policy root's grants and the bound capability summary; labels and metadata are never consulted) | `crates/sley-policy/src/candidate_validation.rs` phase 9; `crates/sley-policy/src/lib.rs` capability tokens | `capability_issuance_requires_policy_grant_and_bounded_budget`, `t22_capability_forgery_fails_closed`, `t23_capability_replay_expiry_and_budget_fail_closed`, `t24_capability_scope_workspace_effect_and_adapter_confusion_fail_closed`, `grant_denial_and_empty_grants_do_not_imply_authority`; `t48_authority_like_labels_do_not_change_policy_grants` checks the same authority-like labels under allowed and denied protected grants and verifies that labels and policy roots remain unchanged |
