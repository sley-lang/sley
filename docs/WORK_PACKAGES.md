...
| S20-700 | affected package | Vulcan | `fuzz/`; adversarial fixtures | bounded schema slice present; full finding register deferred | shallow green suite | 512-input SLEYEP01 smoke and exact no-fallback regression pass; persistent harnesses and remaining required surfaces are absent | `scripts/check_schema_fuzz_slice.py` + `make fuzz-smoke` | every phase/GA blocker; not complete |
