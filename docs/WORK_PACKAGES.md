...
| S20-350 | 345 | Merlin | `sley-mutate` | decomposed candidate/precondition construction; closed host-value slice landed | underbound, opaque-byte, or runtime-type-selected candidate | manifest generation now closes 18 body structs, 18 entity-body variants, and 75 typed field variants with canonical entity-ID sets; binary codecs, 179 descriptor bindings, preconditions, records, and builder remain deferred | host-model drift/structure/unit check; future exact codec/binding/digest/stale-preimage matrix | M3 blocker; host values alone cannot construct a candidate |
