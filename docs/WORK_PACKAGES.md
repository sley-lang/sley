...
| S20-280 | 230,270 | Merlin | `sley-adapter`,`sley-vm` | adapter protocol | escape/impersonation | confined deterministic adapters enforce identity/budgets | adapter adversarial | M2 security gate |
| S20-290 | 240,270 | Merlin | `sley-vm`,`sley-conformance` | execution/test reports | nondeterministic evidence | complete versioned reports and stable digests | report conformance | M2 exit |
| S20-300 | 250 | Merlin | `sley-query` | index snapshot | cache becomes authority | indexes reproduce from root and discard safely | index rebuild property | M3 blocker |
| S20-310 | 300 | Merlin | `sley-query` | typed query | query explosion | every query class exact, ordered, and bounded | query unit/adversarial | M3 blocker |
