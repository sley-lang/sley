...
| S20-350 | 345 | Merlin | `sley-mutate`; low-level `sley-scb1` cursor foundation | decomposed candidate/precondition construction; closed host-value and typed-binding slices landed | underbound, opaque-byte, or runtime-type-selected candidate | manifest generation closes 18 body structs, 18 entity-body variants, 75 typed field variants, canonical entity-ID sets, and exact closed kinds for all 179 immutable descriptors; a strict schema-neutral SCB cursor supports later private codecs; complete binary codecs, preconditions, records, and builder remain deferred | host/binding drift/structure/unit check plus SCB cursor conformance; future exact codec/digest/stale-preimage matrix | M3 blocker; typed admission and low-level decoding cannot construct a candidate |
...
| S20-700 | affected package | Vulcan | `fuzz/`; adversarial fixtures | bounded schema, adapter-binding, and object-store symlink slices present; full finding register deferred | shallow green suite | 512-input SLEYEP01 smoke, exact decoder no-fallback, pre-charge state/effect/adapter bindings, and root/fan-out symlink confinement regressions pass; persistent harnesses and remaining required surfaces are absent | focused checkers + `make fuzz-smoke` + `make adversarial` | every phase/GA blocker; not complete |
