...
| S20-700 | affected package | Vulcan | `fuzz/`; adversarial fixtures | bounded schema and adapter-binding slices present; full finding register deferred | shallow green suite | 512-input SLEYEP01 smoke, exact decoder no-fallback, and pre-charge state/effect/adapter binding regressions pass; persistent harnesses and remaining required surfaces are absent | `scripts/check_schema_fuzz_slice.py` + `make fuzz-smoke` + `make adversarial` | every phase/GA blocker; not complete |
