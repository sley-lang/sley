...
def generate_seed_corpus() -> tuple[int, int]:
    ...
    # Repair round 7c: length-ladder variants exercise the trailing-data
    # and limit paths up to MAX_LEN (padding a valid preimage keeps the
    # header well-formed while the length is novel).
    for padded_length in (256, 512, 1024, 2048):
        if padded_length > len(canonical):
            seeds.append(canonical + b"\x00" * (padded_length - len(canonical)))
