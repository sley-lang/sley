...
FLAG_BASES = 4
# Fact lanes select the target's facts-from-bytes paths (bit 2 entry
# points, bit 3 dependency roots, bit 4 bound entities; 28 selects all
# three): 4 bases x 5 fact lanes = 20 flag lanes per request.
...
def main() -> int:
    ...
    evidence: dict[str, object] = {
        ...
        "flag_lanes": FLAG_BASES * len(FACT_LANES),
...
def encode_entity(entity: dict) -> bytes:
    ...
    if kind == 1:
        ...
        raise SystemExit(
            "kind-8 Operation reached encode_entity: the target reads two "
            "ids (block, immediate entity) for kind 8, so extend this "
            "branch for its fields instead of silently misaligning"
        )
    elif kind == 10:
