...
def main() -> int:
    ...
    if arguments.runs < 0 or arguments.timeout <= 0 or arguments.build_timeout <= 0:
    ...
    runs_floor = max(arguments.runs, corpus_file_count + 256)
    ...
    evidence["owner_lib_sancov_symbols"], evidence["rlib_linkage"] = (
        owner_lib_sancov_symbols(build_record=build, env=environment)
    )
    ...
    if evidence["owner_lib_sancov_total"] == 0 or evidence["owner_lib_sancov"] == 0:
        evidence["result"] = "FAIL"
        evidence["duration_seconds"] = round(time.monotonic() - started, 3)
        evidence.setdefault("problems", []).append(f"owner-lib-sancov-missing:{OWNER_RLIB}")  # type: ignore[union-attr]
        evidence["duration_seconds"] = round(time.monotonic() - started, 3)
        write_evidence(evidence)
        return 1
    ...
    fuzz = run(fuzzer_command(runs=runs_floor), timeout=arguments.timeout)
    ...
    if (
        ...
        evidence["result"] = "FAIL"
        evidence.setdefault("problems", []).append(  # type: ignore[union-attr]
