...
def main() -> int:
    ...
    if not arguments.manual and arguments.runs < corpus_count:
        parser.error(f"--runs ({arguments.runs}) must cover the corpus ({corpus_count})")
    ...
    if evidence["owner_lib_sancov_total"] == 0:
        evidence["result"] = "FAIL"
        evidence["duration_seconds"] = round(time.monotonic() - started, 3)
        evidence.setdefault("problems", []).append("owner-lib-sancov-missing")  # type: ignore[union-attr]
        write_evidence(evidence)
        return 1
