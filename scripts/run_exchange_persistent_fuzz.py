#!/usr/bin/env python3
"""Build and run the S20-540 repository-exchange persistent libFuzzer target."""
...
RUNTIME = ROOT / "evidence/runtime/s20-700-exchange-import-libfuzzer"
CORPUS = RUNTIME / "corpus"
ARTIFACTS = RUNTIME / "artifacts"
EVIDENCE = RUNTIME / "evidence.json"
TARGET_DIR = RUNTIME / "target"
FUZZER = TARGET_DIR / "release/repository_exchange_importer"
...
def generate_seed_corpus() -> int:
    ...
    if vector.get("contract") != "sley2-repository-exchange-v1":
        raise SystemExit("repository-exchange fixture contract drifted")
    entry = vector["vectors"][0]
    canonical = bytes.fromhex(entry["exchange_hex"])
    if len(canonical) != entry.get("stored_bytes"):
        raise SystemExit("repository-exchange fixture length drifted")
    if not canonical.startswith(b"SLEYSCB1") or len(canonical) > MAX_PAYLOAD_LEN:
        raise SystemExit("repository-exchange fixture structure exceeds target bounds")
