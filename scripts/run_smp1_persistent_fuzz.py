...
def generate_seed_corpus() -> tuple[int, int]:
    ...
    # Repair round 7h (frame-identity lane 3): negotiation executes
    # through check_frame's Hello arm (fixture hello frames decode to
    # Hello records there and run the shared negotiation oracle), and
    # multi-frame concatenated pairs reach reassemble_stream beyond the
    # single-frame ceiling. Prefix-stripped bodies are not hello records
    # and were removed: they only exercised the rejection arm while
    # claiming the negotiation lane. These are harness-side corpus
    # entries; the conformance fixtures are untouched.
