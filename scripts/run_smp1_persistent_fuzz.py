...
def generate_seed_corpus() -> tuple[int, int]:
    ...
    # Repair round 7 (frame-identity lane 3): pin the negotiation lane-2 path
    # with prefix-stripped hello bodies, and reach multi-frame reassembly
    # (beyond the single-frame ceiling) with concatenated frame pairs. These
    # are harness-side corpus entries; the conformance fixtures are untouched.
    for hello in accepted["hellos"].values():
        body = bytes.fromhex(hello["frame_hex"])[8:]
