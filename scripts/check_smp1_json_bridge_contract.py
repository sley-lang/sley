...
def main() -> int:
    ...
    # Version 3 overrides the frozen reserved 601/602/605/606/607 rows with
    # the live native rows at 46/44 with two reserved; the frozen
    ...
    if v3_table.get("method_count") != 46 or v3_table.get("reserved_count") != 2:
    ...
    v3_reserved = sorted(
        method.get("tag")
        for method in v3_table.get("methods", [])
        if method.get("reserved", False)
    )
    if v3_reserved and v3_reserved != [305, 503]:
        problems.append("methods-table-v3:reserved-tags")
