...
def scope_generation(short: str) -> int:
    ...
    Counted across the public-history cut: an archived commit counts its
    archived ancestors, a public commit its public ancestors plus the
    archived tip's (the public root continues that tip)."""
    if short not in _generation:
        import subprocess

        if history_ledger.is_archived(short):
            _generation[short] = len(history_ledger._archived_ancestors(history_ledger.resolve(short)))
        else:
            done = subprocess.run(["git", "rev-list", "--count", short], cwd=ROOT, capture_output=True, text=True, check=False)
            tip = history_ledger.archived_tip()
            carried = len(history_ledger._archived_ancestors(tip)) if tip else 0
            _generation[short] = int(done.stdout.strip()) + carried if done.returncode == 0 and done.stdout.strip().isdigit() else -1
...
def strictly_later_scope(later_short: str, earlier_short: str) -> bool:
    ...
    and across the public-history cut via the archived-history ledger):
    ...
    if key not in _scope_order:
        _scope_order[key] = history_ledger.is_ancestor(earlier_short, later_short)
