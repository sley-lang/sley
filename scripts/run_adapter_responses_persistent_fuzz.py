#!/usr/bin/env python3
"""Build and run the scoped restricted-adapter response fuzz target."""
...
RUNTIME = ROOT / "evidence/runtime/s20-700-adapter-responses-libfuzzer"
CORPUS = RUNTIME / "corpus"
ARTIFACTS = RUNTIME / "artifacts"
EVIDENCE = RUNTIME / "evidence.json"
TARGET_DIR = RUNTIME / "target"
FUZZER = TARGET_DIR / "release/adapter_responses"
CLANG = "clang-18"
...
def main() -> int:
    ...
    evidence: dict[str, object] = {
        ...
        "reference_adapter_kind_count": KIND_COUNT,
        "generic_replay_response_schema_count": RESPONSE_SCHEMA_COUNT,
        "mutation_class_count": MUTATION_CLASS_COUNT,
        "max_mutations_per_input": 4,
        "max_input_bytes": MAX_INPUT_LEN,
        "max_payload_bytes": 32,
        "max_collection_items": 4,
        "corpus_count": corpus_count,
        "synthetic_seed_grammar": "bounded fuzz-only typed adapter fixtures v1",
...
def generate_seed_corpus() -> tuple[int, int]:
    ...
    for value in range(256):
        length = 2 + (value % 31)
        seeds.append(bytes((value + (offset * 41)) % 256 for offset in range(length)))

    for kind in range(KIND_COUNT):
        for response_schema in range(RESPONSE_SCHEMA_COUNT):
            for response_arm in range(2):
                seeds.append(
                    bytes([kind, response_schema, 0]) + bytes([response_arm]) * 64
                )
        for mutation in range(MUTATION_CLASS_COUNT):
            seeds.append(bytes([kind, 0, 1]) + bytes([mutation]) * 64)
