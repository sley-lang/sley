...
# Toolchain components statically linked into the musl binary, with the
# notices vendored under licenses/toolchain/ (see its README for sources).
TOOLCHAIN_NOTICES = (
    ("Rust standard library 1.93.0", "MIT OR Apache-2.0", "licenses/toolchain/rust-1.93.0-COPYRIGHT-library.txt"),
    ("musl libc 1.2.5", "MIT", "licenses/toolchain/musl-1.2.5-COPYRIGHT"),
)
...
def render(packages: list[dict]) -> bytes:
    ...
    lines = [
        ...
        "they are included for completeness. The binary also links the Rust",
        "standard library and musl libc from the toolchain; their notices",
        "follow the crates, under \"Toolchain components\". Sley itself is",
    ...
    lines += ["", RULE, "Toolchain components", RULE]
    for component, license_expression, relative in TOOLCHAIN_NOTICES:
        path = ROOT / relative
        if not path.is_file():
            raise LicenseError(f"missing toolchain notice {relative}")
        body = [line.rstrip() for line in path.read_text(encoding="utf-8").replace("\r\n", "\n").split("\n")]
        while body and not body[-1]:
            body.pop()
        lines += ["", THIN_RULE, component, f"License: {license_expression}", f"File: {relative}", THIN_RULE, ""] + body
    return ("\n".join(lines) + "\n").encode("utf-8")
