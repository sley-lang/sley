...
Usage:
    python3 scripts/verify_s20_530_accepted_state.py [--workdir DIR]
        [--commit SHA] [--source REPO] [--keep]

The accepted state predates the public-history cut, so its commits exist only
in the pre-public archive (`evidence/history/pre-public-history.json`). In a
public clone, pass that archive (the Git bundle, or a clone holding it) as
`--source`; the default source is this repository.
...
def main() -> int:
    ...
    os.umask(0o022)
    run((GIT, "clone", "--quiet", "--no-hardlinks", arguments.source, str(clone)), cwd=ROOT)
