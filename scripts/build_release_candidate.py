...
# license files, the third-party license texts and their generator, the SBOM
# inventory, and the conformance subset. S20-730
...
# The statically linked binary redistributes the locked third-party crates,
# so their license texts and copyright notices ship beside it. Staging copies
# the tracked file generated by scripts/generate_third_party_licenses.py; the
# build regenerates it from the cargo registry first and refuses on drift.
THIRD_PARTY_LICENSES = ROOT / third_party.OUTPUT_NAME
...
def verify_third_party_licenses(timeout: int) -> None:
    """Refuse unless the tracked THIRD_PARTY_LICENSES is the exact rendering
    of the locked crate sources.

    `cargo fetch --locked` first fills the registry for every platform in
    Cargo.lock, not only the release target, so the offline regeneration (and
    the later `cargo metadata --offline` of the supply-chain step) works on a
    fresh or partial cache.
    """
    fetched = run(["cargo", "fetch", "--locked"], cwd=ROOT, timeout=timeout)
    if fetched.returncode != 0:
        raise PackageError(PackageErrorCode.BUILD_FAILED, "cargo fetch --locked: " + fetched.stderr[-400:])
    try:
        rendered = third_party.render(third_party.collect(ROOT))
    except third_party.LicenseError as error:
        raise PackageError(PackageErrorCode.BUILD_FAILED, str(error)) from error
    if not THIRD_PARTY_LICENSES.is_file() or THIRD_PARTY_LICENSES.read_bytes() != rendered:
        raise PackageError(
            PackageErrorCode.INTERNAL_INVARIANT,
            f"{third_party.OUTPUT_NAME} differs from the locked crate sources "
            f"(regenerate with {third_party.GENERATOR})",
        )
...
def stage_artifact(
    ...
    *,
    ...
    try:
        indexed = third_party.indexed_packages(third_party_bytes)
    except (third_party.LicenseError, ValueError, UnicodeDecodeError) as error:
        raise PackageError(PackageErrorCode.INTERNAL_INVARIANT, f"{third_party.OUTPUT_NAME}: {error}") from error
    inventoried = sorted(
        (package["name"], package["version"])
        for package in inventory.get("packages", [])
        if package.get("ecosystem") == "cargo" and not package.get("workspace")
    )
    if indexed != inventoried:
        raise PackageError(
            PackageErrorCode.INTERNAL_INVARIANT,
            f"{third_party.OUTPUT_NAME} packages differ from the inventoried third-party crates",
        )
    (stage / third_party.OUTPUT_NAME).write_bytes(third_party_bytes)
    licenses = {
        ...
        "third_party_licenses": {
            ...
            "sha256": sha256_bytes(third_party_bytes),
...
def build_candidate_stages(evidence: dict, *, timeout: int, keep: bool) -> dict:
    ...
    try:
        verify_third_party_licenses(timeout)
