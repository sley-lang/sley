...
Since the public-history cut, the four historical commits exist only in the
archive (`evidence/history/pre-public-history.json`). Their ancestry is then
read through `history_ledger`, and what step 2 read at the acceptance commit
comes from the frozen record `evidence/history/s20-530-acceptance-anchor.json`,
pinned by digest here; a path whose text the export sanitizer rewrote must
carry its recorded exported blob instead.
...
MAKEFILE = "Makefile"
ACCEPTED_STATE_RECORD = "evidence/history/s20-530-acceptance-anchor.json"
ACCEPTED_STATE_RECORD_SHA256 = (
    "e0bd2348a5b7180fdf5914b7599c1df369481194e2d12cf25e1124537392709a"
)
...
SELF_PINNED_PATHS = {
    ...
    "scripts/verify_s20_530_accepted_state.py": "735605d6457574f792e4368a3ccf721d1ddb3b43b4972c41dc3cb2e72b76c1a3",
}
MAKEFILE_MARKERS = (
...
def require_ancestor(commit: str, label: str) -> None:
    if not history_ledger.is_ancestor(commit, "HEAD"):
...
def accepted_state_record() -> dict:
    if working_tree_sha256(ACCEPTED_STATE_RECORD) != ACCEPTED_STATE_RECORD_SHA256:
        fail(f"{ACCEPTED_STATE_RECORD} differs from its pinned digest")
    record = json.loads((ROOT / ACCEPTED_STATE_RECORD).read_text(encoding="utf-8"))
    if record.get("acceptance_commit") != ACCEPTANCE_COMMIT:
        fail("accepted-state record names a different acceptance commit")
    if record.get("archived_tip") != history_ledger.archived_tip():
        fail("accepted-state record names a different archived tip")
    if record.get("log_dir") != LOG_DIR:
        fail("accepted-state record names a different log directory")
    return record


def accepted_logs(record: dict | None) -> list[str]:
    if record is None:
        return tree_paths(ACCEPTANCE_COMMIT, LOG_DIR)
    logs = record.get("logs")
    if not isinstance(logs, list) or logs != sorted(set(logs)):
        fail("accepted-state record log set is malformed")
    paths = record.get("paths")
    if not isinstance(paths, dict) or sorted(paths) != sorted(
        (*FROZEN_AUTHORITY_PATHS, *logs)
    ):
        fail("accepted-state record path set differs from the frozen paths")
    return logs


def accepted_entry(record: dict | None, relative: str) -> tuple[str, str]:
    """The blob HEAD must carry for `relative`, and its acceptance tree mode."""
    if record is None:
        return blob_id(ACCEPTANCE_COMMIT, relative), tree_mode(ACCEPTANCE_COMMIT, relative)
    entry = record["paths"][relative]
    if not isinstance(entry, dict):
        fail(f"accepted-state record entry for {relative} is malformed")
    return entry.get("sanitized_public_blob", entry.get("acceptance_blob")), entry.get("mode")


def require_frozen(record: dict | None, relative: str) -> None:
    accepted, accepted_mode = accepted_entry(record, relative)
...
def main() -> None:
    ...
    if history_ledger.resolve(FREEZE_COMMIT) != FREEZE_COMMIT:
        fail("freeze commit is missing")

    record = accepted_state_record() if history_ledger.is_archived(ACCEPTANCE_COMMIT) else None
    logs = accepted_logs(record)
    ...
    for relative in (*FROZEN_AUTHORITY_PATHS, *logs):
        require_frozen(record, relative)
