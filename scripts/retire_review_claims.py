...
def strictly_later(later: str | None, earlier: str | None) -> bool:
    """`later` is a strict descendant of `earlier` in this repository or,
    across the public-history cut, in the archived-history ledger."""
    if not later or not earlier or later.startswith(earlier) or earlier.startswith(later):
        return False
    key = (earlier, later)
    if key not in _ancestry:
        _ancestry[key] = history_ledger.is_ancestor(earlier, later)
...
def resolve_scope(short: str) -> str | None:
    """The full commit id a transcript's scope stamp names, in this
    repository or the archived-history ledger (None when unknown)."""
    if short not in _resolved:
        _resolved[short] = history_ledger.resolve(short)
