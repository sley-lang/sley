#!/usr/bin/env python3
"""Build and run the S20-510 semantic-delta decoder persistent libFuzzer target."""
...
RUNTIME = ROOT / "evidence/runtime/s20-700-semantic-delta-libfuzzer"
CORPUS = RUNTIME / "corpus"
ARTIFACTS = RUNTIME / "artifacts"
EVIDENCE = RUNTIME / "evidence.json"
TARGET_DIR = RUNTIME / "target"
FUZZER = TARGET_DIR / "release/semantic_delta_decoder"
...
def generate_seed_corpus() -> tuple[int, int]:
    ...
    if accepted.get("contract") != "sley2-semantic-comparison-v1":
        raise SystemExit("semantic-comparison fixture contract drifted")
    stored = [bytes.fromhex(vector["stored_hex"]) for vector in accepted["vectors"]]
    for vector, canonical in zip(accepted["vectors"], stored):
        if len(canonical) != vector.get("stored_bytes") or not canonical.startswith(b"SLEYSCB1"):
            raise SystemExit("semantic-comparison fixture structure drifted")
