...
- `bench/raw/runner.py:11-20` imports only `fcntl, hashlib, json, os, re, stat, datetime, enum, pathlib, typing`. No `subprocess/socket/requests/urllib/http` (checker AST-gates these; confirmed by read). No `os.system/os.popen/Popen/run_command//home/dev/sley` strings.
