...
register against the current frontier. A successful row advances both the
inventory index and dense-register frontier with checked integer addition.
The second row can therefore consume the result register derived for the first
row. Completion returns
`Result<Tuple<UInt64, UInt32>, UInt32>` containing the processed operation
count and final register frontier.
...
condition proves it unreachable for a conforming VM.

## Native comparison and gate

`lower_ordered_boolean_inventory_matches_native_frontier` compares the Sley
result for `BoolAnd(0,1); BoolNot(2)` with `sley_vm::lower_function`: two
instructions and final register frontier four. It also runs the same request
twice for determinism and proves the empty inventory preserves frontier two.
...
SSMC closure objects. This slice returns the derived count and register
frontier; it does not yet return the full instruction vector, traverse block
or function inventories, lower terminators, build a callee table, emit
`SLEYBC02`, assemble `EXEC_PACKAGE_V2`, or define a `BuildError` vocabulary.
