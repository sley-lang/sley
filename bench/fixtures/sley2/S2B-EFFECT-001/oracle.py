#!/usr/bin/env python3
"""S3 G3 oracle: bench/fixtures/sley2/S2B-EFFECT-001 (excluded).

No execution fixture exists by contract. This oracle reproduces the pinned
refusal against the real engine: it attempts the effect-request operation
through the real restricted lowering path (`s3_g3_effect_refusal_pin` in
crates/sley-vm/tests/s3_g3_perf.rs) via `cargo test --offline` and requires
the refusal to be exactly `VM_LOWER_OPCODE_UNSUPPORTED` — the same code
recorded in excluded.json. Verdict rejected/exit 1 only then.
...
ARM = "sley2"
PIN_TEST = "s3_g3_effect_refusal_pin"
TIMEOUT_S = 300
...
def main(argv):
    ...
    emit("rejected", "VM_LOWER_OPCODE_UNSUPPORTED",
         "E7 effect execution refused: VM_LOWER_OPCODE_UNSUPPORTED")
    sys.exit(1)
