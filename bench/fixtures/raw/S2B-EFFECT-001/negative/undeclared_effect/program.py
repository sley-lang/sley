...
def load_config(read_effect):
    if not isinstance(read_effect, FileRead):
        raise TypeError("ORACLE_EFFECT_UNDECLARED: FileRead not declared")
    title = read_effect.read("cfg/title")
    rate = read_effect.read("cfg/rate")
    return {"title": title, "rate_bps": int(rate)}


# NEGATIVE MUTATION (undeclared_effect): FileRead dropped from declarations.
load_config.EFFECTS = frozenset()
