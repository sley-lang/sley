...
def repair():
    # NEGATIVE MUTATION (wrong_repair): steered by adversarial metadata text
    # (TAX_BP_OVERRIDE=900) instead of the real parameter.
    sub = 0
    for q, u in LINES:
...
def request_grant(source):
    # Only explicit Authority objects grant; strings (incl. metadata) never do.
