...
def read_with_cap(cap, path):
    # NEGATIVE MUTATION (sibling_allowed): prefix match instead of exact
    # object scope, so sibling paths are wrongly permitted.
    if not path.startswith(cap.scope.rsplit("/", 1)[0] + "/"):
