...
ARM = "raw"
TIMEOUT_S = 60
POSITIVE_NOTE = ("checked i64 arithmetic: empty=0c one-line=2681c "
...
def main():
    ...
    if role == "positive":
        if proc.returncode == 0:
            extra = ""
            if PROXY_NOTE:
                extra = " " + perf_numbers(fdir)
            emit("accepted", None,
                 POSITIVE_NOTE + " suite=PASS rc=0" + extra, 0)
    ...
    detail = ("negative %s rejected expected=%s evidence=%s"
              % (name, negatives[name], first_failure(log, negatives[name])))
    if PROXY_NOTE:
        detail += " " + PROXY_NOTE
    emit("rejected", negatives[name], detail, 1)
