...
ARM = "raw"
TIMEOUT_S = 120
POSITIVE_NOTE = ("before/after identical digest; reduction>=30% recorded")
PROXY_NOTE = ("proxy=deterministic shared step counter, not wall-clock "
              "or VM bytecode")
...
def perf_numbers(fdir):
    try:
        import importlib.util
        spec = importlib.util.spec_from_file_location(
            "perf_program", str(fdir / "program.py"))
        mod = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(mod)
        d = mod.run_demo()
        return ("proxy=deterministic shared step counter (not wall-clock); "
                "before=%d after=%d reduction=%.1f%% digests_match=%s"
                % (d["before_steps"], d["after_steps"],
                   100.0 * d["reduction"],
                   d["digest_before"] == d["digest_after"]))
    except Exception as e:
        return "proxy metrics unavailable: %s" % str(e)[:100]
...
def main():
    ...
    if role == "positive":
        if proc.returncode == 0:
            emit("accepted", None,
                 POSITIVE_NOTE + " suite=PASS rc=0 " + perf_numbers(fdir), 0)
        emit("rejected", "ORACLE_POSITIVE_FAILURE",
             "positive suite FAILED rc=%d evidence=%s %s"
             % (proc.returncode, first_failure(log), PROXY_NOTE), 1)
    ...
    emit("rejected", negatives[name],
         "negative %s rejected expected=%s evidence=%s %s"
         % (name, negatives[name], first_failure(log, negatives[name]),
            PROXY_NOTE), 1)
