...
Structural SSMC1 data model for all eighteen epoch-1 entity bodies and the
restricted S20-250 canonical fingerprint encoder.
...
restricted epoch-1 S20-240 profile, and the full S20-250 `Workspace`,
`Package`, `Namespace`, `EntryPoint`, `PolicyBinding`, and
`DependencyBinding` bodies with `EntryExposure`. It performs no schema selection, SCB
