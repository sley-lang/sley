...
#[test]
fn v3_serve_routes_live_report_paging_past_reservation() {
    use std::cell::RefCell;
    use std::rc::Rc;

    // A v3 client opens a version 3 session, then calls the live 605 with
    // a well-formed body naming an unknown token: the call must travel
    // past reservation into the live handler and refuse as an invalid
    // token, never as reserved (40007). Reservation is gone for this tag;
    // only the minted capability stands in the way.
    let (_temp, path) = repository("cli-v3-profile-paging-live");
    let repo = path.to_str().unwrap().to_string();
    let client = offered_v3();
    let (_, handshake) = sley_protocol::negotiate_identity_versioned(&client, &client).unwrap();
    let mut input = encode_hello_frame(&client).unwrap().bytes;
    ...
    let probe = sley_scb1::encode_record(&[
        (1, vec![0xE1; 32]),
        (2, sley_scb1::encode_uvar(0)),
        (3, sley_scb1::encode_uvar(64)),
    ])
    .unwrap();
    let report_path = path
        .parent()
        .unwrap()
        .join("v3-paging-live-report.json")
        .to_str()
        .unwrap()
        .to_string();
    let script = Rc::new(RefCell::new(Script::with_probe(
        input,
        Method::TestsReportRead.tag(),
        probe,
    )));
    let mut stdin = ScriptIn(Rc::clone(&script));
    let mut stdout = ScriptOut(Rc::clone(&script));
    ...
    assert_eq!(refused.protocol_version, PROTOCOL_VERSION_V3);
    let failure = ProtocolFailure::decode(&refused.body).unwrap();
    assert_eq!(failure.symbol, "NATIVE_TOKEN_INVALID");
    let report: Value =
        serde_json::from_str(&std::fs::read_to_string(&report_path).unwrap()).unwrap();
    assert_eq!(report["contract"], "sley2-cli-report-v3");
    assert_eq!(report["selected_protocol_version"], 3);
